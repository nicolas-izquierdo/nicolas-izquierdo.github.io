document.addEventListener("DOMContentLoaded", () => {
  ...
  // baseurl robusto (user-site = "", project-site = "/repo")
  ...
  async function loadAndPickMovie() {
    ...
    if (!Array.isArray(movies) || movies.length === 0) throw new Error("movies.json is empty or not an array.");
  ...
  trigger.addEventListener("click", async (ev) => {
    ...
    try {
      ...
      card.innerHTML = `
        <div class="movie-card-header">Today’s movie recommendation!</div>
        <p class="movie-desc">Couldn’t load the movie list. Check the JSON path.</p>
